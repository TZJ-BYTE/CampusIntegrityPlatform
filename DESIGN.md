...
- 功能：查看数据目录、备份与恢复 user.db、查看版本与设备标识、基础诊断信息
- 说明：
  - 不提供“手动导入内容库/导入更新包”等入口（内容更新走自动更新链路）
  - 内容与用户数据分离：内容更新不覆盖 user.db
  - 若学校确有离线分发需要，可由管理员侧工具/运维流程处理，不占用普通用户 UI（阶段性不做）
...
### 4.8 UI 与交互设计（主流软件模式，离线优先 + 云同步）

本节给出最终 UI/信息架构规范：**登录为弹窗**、**右上角头像为唯一入口**、**设置为弹窗而非独立页面**。目标是对齐主流软件的“云存储式体验”，同时不破坏离线优先。

#### 4.8.1 关键原则

- 默认可用：首次打开直接进入首页/内容，离线功能完整可用
- 渐进增强：只有当用户想用云能力（同步/跨端）时才引导登录
- 云后台化：同步自动运行；主界面只呈现状态（未连接/同步中/异常/已连接）
- 少打断：登录、账号、设置、诊断均采用弹窗/抽屉（Overlay）承载，不切换主页面上下文
- 单入口：右上角“用户头像”作为唯一系统入口（账号/设置/诊断/用户信息均从这里进入）

#### 4.8.2 页面类型与范围

**A. 业务路由页（Router Pages，只放学习内容）**

- 首页/概览（入口总览、学习摘要、今日故事、快捷入口）
- 场所 / 案例 / 法规 / 故事 / 竞答 / 收藏（各模块主页面）
- 说明：这些页面不出现登录表单、服务器地址、同步按钮、设置按钮；避免“学习页变成系统页”。

**B. Overlay（弹窗/抽屉，不进入路由）**

- 首次欢迎弹窗（Onboarding Modal，仅首次出现）
- 登录弹窗（Auth Modal：登录/注册/找回密码可选）
- 账号面板（Account Sheet：云状态、待同步、切换账号、退出、诊断入口）
- 用户信息弹窗（Profile Modal：头像/昵称/可选信息）
- 设置弹窗（Settings Modal：本地数据、备份恢复、外观）
- 诊断弹窗（Diagnostics Modal：同步错误摘要、最近成功/失败、导出诊断信息）

> 说明：Overlay 关闭后返回原页面，不改变路由历史；用户感知为“开启/管理服务”，不是“离开学习流程去办手续”。

#### 4.8.3 顶栏（AppShell）规范

- 左侧：品牌（可弱化）、当前栏目/页面标题
- 中间：学习模块导航（概览/场所/案例/法规/故事/竞答/收藏）
- 右侧：**用户头像按钮（唯一入口）**
  - 头像旁可选显示一条轻量云状态文本（非按钮）：例如“云 未连接 / 同步中 / 异常 / 已连接”
  - 移除“刷新”按钮：避免误导用户“同步需要手动刷新”

#### 4.8.4 用户头像（Avatar）设计
...
- 视觉：中心白色高光 → 向外扩散为主色（或双主色）径向渐变
- 主色来源（优先级）：
  1) 用户自选颜色  
  2) 基于 `userId`（已登录）或 `deviceId`（未登录）哈希生成稳定颜色  
- 可选叠字：昵称首字/“廉”，以保证弱对比背景下可识别

#### 4.8.5 头像菜单（Popover Menu）信息架构

点击头像弹出菜单（建议从高频到低频）：

- 账号与云同步…（打开 Account Sheet）
- 用户信息…（打开 Profile Modal）
- 设置…（打开 Settings Modal）
- 诊断…（打开 Diagnostics Modal）
- 关于…（版本/设备标识/许可证，可选）
- 分隔线
- 退出登录（仅已登录时显示；退出后仍可离线使用，数据保留本地）

菜单右侧或菜单项上可显示“异常红点/角标”，用于提示云同步异常但不强打断。

#### 4.8.6 账号面板（Account Sheet）内容规范
...
- 未登录：登录（打开 Auth Modal）
- 已登录：切换账号（退出并立即打开 Auth Modal）、退出登录
- 异常状态：查看原因（展开错误摘要或进入诊断）
- 高级设置（折叠）：服务器地址/调试信息（仅高级用户可见）

#### 4.8.7 设置弹窗（Settings Modal）范围

设置弹窗只承载“本地与外观”，不承载“账号与同步”：

- 本地数据：数据目录（只读）、便携/系统模式说明
- 备份与恢复：一键备份 user.db、从备份恢复、备份列表管理
- 外观（可选）：主题色、字号、动效开关
- 说明：账号/服务器地址/同步诊断均不放在设置里，避免“设置页杂物化”。
...
3) 用户选择：
   - “先离线使用” → 关闭弹窗 → 留在当前页面  
   - “登录开启云同步” → 关闭欢迎弹窗 → 打开登录弹窗 → 成功后回到原页面  

**日常登录/管理**

- 任意页面点击头像 → 菜单 → 账号与云同步 →（未登录则登录弹窗）→ 成功后回到原页面

**异常提示**

- 顶栏轻量显示“云 异常”或头像红点  
- 用户点击头像 → 账号面板显示“错误摘要/查看原因”  
- 需要排查时进入诊断弹窗（导出诊断信息用于反馈）
...
Overlay（弹窗/抽屉）:
  - Onboarding Modal（首次）
  - Auth Modal（登录）
  - Account Sheet（云状态/切换账号/诊断入口）
  - Profile Modal（头像/昵称）
  - Settings Modal（本地/备份/外观）
  - Diagnostics Modal（错误/导出）
```

如需更“主流”的移动端感受：Account/Settings/Diagnostics 推荐抽屉（Sheet），登录推荐居中弹窗或全屏 Sheet（取决于屏幕尺寸）。
...
里程碑的执行拆解与验收清单见 [DEVELOPMENT_PLAN.md](file:///d:/project/rustproject/CampusIntegrityPlatform/DEVELOPMENT_PLAN.md)。
